...
<div class="row justify-content-evenly my-6">
    ...
    <div class="col-12 col-lg-4">
        <form action="{{route('send.email')}}" method="post" id="formulario_contacto">
            @csrf
            <x-honeypot />
            ...
            <div class="row justify-content-center mt-4">
                <button type="submit" class="btn btn-orange rounded-0" id="submit_button" @if (session('contact_message')) disabled @endif >{{__('Enviar')}}</button>
            </div>
...
<script>
  document.addEventListener('DOMContentLoaded', function () {
    const cform = document.getElementById('formulario_contacto');
